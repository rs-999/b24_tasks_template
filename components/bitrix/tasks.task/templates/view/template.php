...
$direction = $connection->query("
    SELECT *
    FROM c_directions WHERE task_id = '".$taskData['ID']."'
")->fetch();

$process = $connection->query("
    SELECT *
    FROM c_processes WHERE task_id = '".$taskData['ID']."'
")->fetch();

$project = $connection->query("
    SELECT *
    FROM c_projects WHERE task_id = '".$taskData['ID']."'
")->fetch();

$subTasks = $connection->query("
    SELECT ID, TITLE
    FROM b_tasks WHERE PARENT_ID = '".$taskData['ID']."'
")->fetchALL();
...
if (isset($direction['task_id'])) {
  $checkDirection = true;

  $taskIds = array_column($subTasks, 'ID');
  $ids = array_map('intval', $taskIds);
  $idsSql = implode(',', $ids);

  if ($idsSql) {
    $processes = $connection->query("
      SELECT 
        id,
        task_id,
        TITLE,
        ...
        resources
      FROM c_processes
      WHERE task_id IN ($idsSql)
    ")->fetchALL();
  }
}

if (isset($process['task_id'])) {
  $checkProcess = true;
...
<style>
.events-row-full > :nth-child(1),
.events-header-full > :nth-child(1)
{
    width: 5%;
    flex-shrink: 0;
}
  ...
  <div class="project-container">
    <div class="project-center-column">
      <div class="project-table">
          ...
          <div class="events-header-full">
              <div>№</div>
              <div>Процесы</div>
              <div>Текущее состояние</div>
              <div>Целевое состояние</div>
              <div>Метрики</div>
              <div>KPI на '.date("Y").' год</div>
              <div>KPI факт</div>
              <div>Ресурсы</div>
          </div>';
          ...
          foreach ($processes as $k => $v) {
            ...
            echo '<div class="events-row-full">';
              echo '<div>'.$matches[0].'</div>';
              echo '<div><a href="/company/personal/user/1/tasks/task/view/'.$v['task_id'].'/?IFRAME=Y&IFRAME_TYPE=SIDE_SLIDER&ta_sec=tasks&ta_sub=task_card&ta_el=title_click/">'.$v['TITLE'].'</a></div>';
...
<script>
    ...
    function changeData(el) {
      el.addEventListener("blur", function ()
      {
        ...
        BX.ajax({
            url: "/local/components/bitrix/tasks.task/templates/lib/process-edit.php",
            method: "POST",
            dataType: "json",
            data: data
        })
    ...
    current_state.forEach(el => {
      changeData(el);
    });

    target_state.forEach(el => {
      changeData(el);
    });
...
if ($checkProcess) {
  preg_match('/^[\d.-]+/', $process['TITLE'], $matches);
    ...
    <div class="project-container">
      ...
      <div class="project-table">
          ...
          foreach ($projects as $k => $v) {
              echo '<div class="events-row-full">';
                echo '<div>
                      <a href="/company/personal/user/1/tasks/task/view/'.$v['task_id'].'/?IFRAME=Y&IFRAME_TYPE=SIDE_SLIDER&ta_sec=tasks&ta_sub=task_card&ta_el=title_click/">'
                          .$v['TITLE'].
                      '</a>
                    </div>
                  </div>';
...
if ($checkProject) {
  ...
  <div class="project-container">
      ...
      <div class="project-left-column">
          <div class="project-table">
              <div class="project-table-title">
                  Текущая фаза: Концепция
              </div>
              <p><u>Заказчик</u> - 
                <a href="/company/personal/user/'.$arResult['DATA']['TASK']['SE_ORIGINATOR']['ID'].'/">
                '.$arResult['DATA']['TASK']['SE_ORIGINATOR']['NAME'].' '.$arResult['DATA']['TASK']['SE_ORIGINATOR']['LAST_NAME'].'
                </a>
              </p>
              <p><u>Куратор</u> - 
                <a href="/company/personal/user/'.$arResult['DATA']['TASK']['RESPONSIBLE_ID'].'/">
                '.$arResult['DATA']['TASK']['RESPONSIBLE_NAME'].' '.$arResult['DATA']['TASK']['RESPONSIBLE_LAST_NAME'].'
                </a>
              </p>
              <p><u>Менеджер</u> - 
                <a href="/company/personal/user/'.$arResult['DATA']['TASK']['SE_ACCOMPLICE'][0]['ID'].'/">
                '.$arResult['DATA']['TASK']['SE_ACCOMPLICE'][0]['NAME'].' '.$arResult['DATA']['TASK']['SE_ACCOMPLICE'][0]['LAST_NAME'].'
                </a>
              </p>
              <p><u>Цель проекта:</u></p>
              <p>
                <div contenteditable="true" id="target" data-type="target" data-task-id="'.$project['task_id'].'">
                  '.$project['target'].'
                </div>
              </p>
              <br>
              <p><u>Контекст:</u></p>
              <p>
                <div contenteditable="true" id="contecst" data-type="contecst" data-task-id="'.$project['task_id'].'">
                  '.$project['contecst'].'
                </div>
              </p>
          </div>
          ...
          <div class="project-table">
              ...
              foreach($projects as $k => $v) {
                ...
                if ($v['PRIORITY'] == 2) {
                  ...
                  echo '<div style="'.$style.'"><a href="/company/personal/user/1/tasks/task/view/'.$v['id'].'/?IFRAME=Y&IFRAME_TYPE=SIDE_SLIDER&ta_sec=tasks&ta_sub=task_card&ta_el=title_click">'.$v['TITLE'].'</a></div>';
                  if (!$v['PLAN_DATE']) {
                    echo '<div contenteditable="true" id="plan_date" data-type="plan_date" data-task-id="'.$v['id'].'" style="'.$style.'"></div>';
                  } else {
                    echo '<div style="'.$style.'">'.$v['PLAN_DATE'].'</div>';
                  }
                  echo '<div style="'.$style.'">'.(new DateTime($v['DEADLINE']))->format('d.m.Y').'</div>';
  ...
  <div class="project-container">
      ...
      <div class="project-left-column">
          <div class="project-table">
              <div class="events-header">
                  <div>Основные показатели проекта</div>
                  <div>Утверждено</div>
                  <div>Факт на '.date("d.m.Y").'</div>
                  <div>Отклонение</div>
              </div>
              ...
              <div class="events-row">
                  <div contenteditable="true" id="kpi" data-type="kpi" data-task-id="'.$project['task_id'].'">
                    '.$project['kpi'].'
                  </div>
                  <div contenteditable="true" id="kpi_date" data-type="kpi_date" data-task-id="'.$project['task_id'].'">
                    '.$project['kpi_date'].'
                  </div>
                  <div contenteditable="true" id="kpi_fact" data-type="kpi_fact" data-task-id="'.$project['task_id'].'">
                    '.$project['kpi_fact'].'
                  </div>
                  <div></div>
              </div>
      ...
      <div class="project-right-column">
          ...
          <div class="project-table">
              ...
              <div contenteditable="true" id="questions" data-type="questions" data-task-id="'.$project['task_id'].'">
                  <pre>'.trim($project['questions']).'</pre>
              </div>
              <br>
          </div>
              ...
              <div class="events-header">
                ...
                foreach($projects as $k => $v) {
                  ...
                  if ($v['PRIORITY'] != 2) {
                    ...
                    echo '<div class="events-row">';
                    echo '<div style="'.$style.'"><a href="/company/personal/user/1/tasks/task/view/'.$v['id'].'/?IFRAME=Y&IFRAME_TYPE=SIDE_SLIDER&ta_sec=tasks&ta_sub=task_card&ta_el=title_click">'.$v['TITLE'].'</a></div>';
                    echo '<div style="'.$style.'"><a href="/company/personal/user/'.$userId.'/">'.$user['NAME'].' '.$user['LAST_NAME'].'</a></div>';
                    echo '<div style="'.$style.'">'.(new DateTime($v['DEADLINE']))->format('d.m.Y').'</div>';
                    echo '<div style="'.$style.'">'.$status.'</div>';
                    echo '</div>';
                  }
                }
  ...
  <script>
    ...
    let contecst = document.getElementById("contecst");
    let questions = document.getElementById("questions");

    let kpi = document.getElementById("kpi");
    let kpi_date = document.getElementById("kpi_date");
    let kpi_fact = document.getElementById("kpi_fact");
    ...
    function changeData(el) {
      el.addEventListener("blur", function ()
      {
        ...
        BX.ajax({
            url: "/local/components/bitrix/tasks.task/templates/lib/project-edit.php",
            method: "POST",
            dataType: "json",
            data: data
        })
    ...
    changeData(target);
    changeData(contecst);
    changeData(questions);

    changeData(kpi);
    changeData(kpi_date);
    changeData(kpi_fact);

    changeData(plan_date);
...
if (!$checkDirection && !$checkProcess && !$checkProject) {
  ...
  if (in_array($arParams["USER_ID"], $allowedUsers, true)) {
    echo '<div class="ui-btn-container" style="margin: 0;">
              <button id="createDirection" class="ui-btn ui-btn-primary" style="transform: scale(0.8);">Направление</button>
              <button id="createProcess" class="ui-btn ui-btn-primary" style="transform: scale(0.8);">Процесс</button>
              <button id="createProject" class="ui-btn ui-btn-primary" style="transform: scale(0.8);">Проект</button>
          </div>';
  }
}
...
BX.ready(function ()
{
      let btnDirection = document.getElementById('createDirection');
      let btnProcess = document.getElementById('createProcess');
      let btnProject = document.getElementById('createProject');
      
      let heightProject = document.querySelector(".project");
      let taskSidebar = document.querySelector(".task-iframe-sidebar");
      let taskContent = document.querySelector(".task-iframe-content");

      if (heightProject) {
        taskSidebar.style.marginTop = heightProject.offsetHeight + 10 +"px";
        taskContent.style.marginTop = heightProject.offsetHeight + 10 +"px";
      }
      ...
      if (btnDirection) {
        ...
        {
            BX.ajax({
                url: '/local/components/bitrix/tasks.task/templates/lib/direction.php',
                method: 'POST',
                dataType: 'json',
                data: data
            })
      ...
      if (btnProcess) {
        ...
        {
            BX.ajax({
                url: '/local/components/bitrix/tasks.task/templates/lib/process.php',
                method: 'POST',
                dataType: 'json',
                data: data
            })
      ...
      if (btnProject) {
        ...
        {
          BX.ajax({
              url: '/local/components/bitrix/tasks.task/templates/lib/project.php',
              method: 'POST',
              dataType: 'json',
              data: data
          })
